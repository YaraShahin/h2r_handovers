...
class HandStabilizationNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('position_tolerance_px', 2000000000000.0)
        ...
        self._window_duration = self.get_parameter('window_duration').value
        ...
        self._history = deque()  # entries: (stamp_sec, (cx, cy))
        ...
        self.get_logger().info(
            ...
            f"(tolerance={self._position_tolerance_px}px over {self._window_duration}s)"
        )
    ...
    def _mask_callback(self, msg: Image) -> None:
        ...
        if self._tracking_start is None:
            self._tracking_start = stamp

        centroid = (float(xs.mean()), float(ys.mean()))
        self._history.append((stamp, centroid))
        self._trim_history(stamp)

        self._publish_status(self._evaluate_stability(stamp))

    def _trim_history(self, now: float) -> None:
        while self._history and now - self._history[0][0] > self._window_duration:
            self._history.popleft()

    def _evaluate_stability(self, now: float) -> str:
        if len(self._history) < self._min_samples:
            return STATUS_UNSTABLE

        if now - self._tracking_start < self._window_duration:
            # Not enough elapsed time covered yet to judge stability.
            return STATUS_UNSTABLE

        positions = np.array([pos for _, pos in self._history])
